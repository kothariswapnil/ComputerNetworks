...
int main ()
{
	...
	ifstream patient1("patient1.txt");						
	string user_name,password,line;
	
	if(patient1.is_open())
	{
		line.clear();
		while(getline(patient1,line))
		{
	...
	{
		cout<<"File patient1.txt not found"<<endl;
	}	
	...
	for(p = servinfo; p != NULL; p = p->ai_next) {
		...
		if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
			close(sockfd);
			perror("patient1: connect");
			continue;
		}
		break;
	}
	if (p == NULL) {
		fprintf(stderr, "patient1: failed to connect\n");
		return 2;
	}
...
void create_Process_UDPSocket(char *docPort){
    ...
    if (( numbytes = sendto(sockfd_UDP, insurance_type, strlen(insurance_type), 0,p->ai_addr,p->ai_addrlen) ) == -1) {
	    ...
	    printf("Phase 3: Patient 1 has dynamic UDP port number %d and IP address %s.\n",port, ipstr);
    }
    freeaddrinfo(servinfo); 				// ALL DONE WITH THIS STRUCTURE
    if ((numbytes = recvfrom(sockfd_UDP, buf, MAXDATASIZE - 1, 0,(struct sockaddr *)&their_addr, &addr_len)) == -1) {
		...
		printf("Phase 3: The cost estimation request from Patient 1 with insurance plan %s has been sent to doctor with port number %s and IP address %s.\n",insurance_type,doc_port,docIPstr );
		printf("Phase 3: Patient 1 receives %s$ estimation from doctor with port number %s and name %s.\n",buf,doc_port,(!strcmp(doc_port,PORT_UDP_DOC1))?"Doctor 1":"Doctor 2" );
...
string readInsuranceFile(){
	...
	ifstream patient1insurance("patient1insurance.txt");						
	string insuranceType,line;
	if(patient1insurance.is_open())
	{
		while(getline(patient1insurance,line))
		{
			insuranceType=line;	
		}
		patient1insurance.close();
	}
	else
	{
		fprintf(stderr, "File patient1.txt not found\n");
	}
